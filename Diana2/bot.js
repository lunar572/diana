...
if (atfk < 15) return;
  ...
  if (message.content.toLowerCase().includes("diana")) {
    message.react("🧝");
  }
  if (message.content.toLowerCase().includes(config.criador)) {
    message.react("😈");
  }
  if (message.content.toLowerCase().includes("599967144066940968")) {
    message.react("😈");
  }
  if (message.content.toLowerCase().includes("mendreik")) {
    message.react("😈");
  }
    ...
    database
      ...
      .then(async function(snap) {
        ...
        if (levelup <= xp) {
          ...
          await message.channel
            .send(
              `🧝 | Parabéns ${message.author},\n o seu level de possuído subiu para **${LEVELXPAZAO}**`
            )
